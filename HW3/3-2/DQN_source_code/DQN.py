...
parser.add_argument("--mem_capacity", default=90000,
                    type=int)  # In paper, size is 1000000
...
parser.add_argument("--epsilon_decay", default=1000000,
                    type=float)  # 500000 1000000 4000000
...
class DQN(object):
    ...
    def save_model(self, path="."):
        torch.save(self.target_net.state_dict(), path +
                   '/q_target_checkpoint_{}.pth'.format(self.interaction_steps))
        torch.save(self.policy_net.state_dict(), path +
                   '/q_policy_checkpoint_{}.pth'.format(self.interaction_steps))
...
class Atari(object):
    ...
    def step(self, action):
        ...
        next_state = torch.cat(
            (self.state[:, 1:, :, :], frame.unsqueeze(0)), axis=1)
